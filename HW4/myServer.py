...
# binary
binary_type_files = set(["pdf", "jpg", "jpeg", "gif", "png", "mp3", "mp4", "mpeg", "wav", "mov"])
...
class ResponseBuilder:
    ...
    def build(self) -> bytes:
        ...
        for header in self.headers:
            response = response + header + NEWLINE
        response_bytes = response.encode('utf-8') + self.content
...
class HTTPServer:
    ...
    def get_request(self, request: Request) -> Response:
        ...
        print("path:" + path)
        #ext = get_file_extension(resource)

        if not file_exists(resource):
            return self.resource_not_found()
        elif not has_permission_other(resource):
    ...
    def head_request(self, request: Request) -> Response:
        ...
        #print("path:" + path)
        if not file_exists(resource):
            return self.resource_not_found_header_only()
        elif not has_permission_other(resource):
            return self.forbidden_access_header_only()
        else:
            return self.ok_response_header_only(resource)
    ...
    def resource_not_found_header_only(self) -> Response:
        """
        Returns 404 not found status and sends back headers.
        """
        builder = ResponseBuilder()
    ...
    # Builds 403 error response
    def forbidden_access(self) -> Response:
        """
        Returns 403 not found status and sends back our 403.html page.
        """
        builder = ResponseBuilder()
    ...
    def forbidden_access_header_only(self) -> Response:
        """
        Returns 403 not found status and sends 403 headers.
        """
        builder = ResponseBuilder()
        builder.set_status("403", "FORBIDDEN")
        builder.add_header("Connection", "close")
    ...
    # Builds http ok response headers
    def ok_response_header_only(self, resource) -> Response:
        """
        Returns 200 ok status and sends back the headers.
        """
        builder = ResponseBuilder()
    ...
    # Builds http ok response
    def ok_response(self, resource) -> Response:
        """
        Returns 200 ok status and sends back content of the page.
        """
        builder = ResponseBuilder()
        builder.set_status("200", "OK")
        builder.add_header("Connection", "close")
        if should_return_binary(get_file_extension(resource)):
            builder.set_content(get_file_binary_contents(resource))
        else:
            builder.set_content(get_file_contents(resource))
        return builder.build()
